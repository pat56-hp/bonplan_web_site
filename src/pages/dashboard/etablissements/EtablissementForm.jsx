...
export default function EtablissementForm() {
  ...
  const getEtablissement = useMutation({
    ...
    onSuccess: ({ data }) => {
      ...
      const oldCommoditesToUpdate = response.commodites?.map(
        (commodite) => commodite.id
      );
        ...
        const updatedHours = hours.map((hour) => {
          if (Object.keys(horaires).includes(hour.label)) {
            ...
            return {
              ...hour,
              checked: true,
              ouverture: createDate(horaire.ouverture),
              fermeture: createDate(horaire.fermeture),
            };
          }
          return hour;
        });
      ...
      const galleries = response.galleries?.map(
        (gallery) => import.meta.env.VITE_API_DOMAIN + gallery.image
      );
  ...
  const submitEtablissement = useMutation({
    ...
    mutationFn: async ({ postUrl, formData }) =>
      sendRequest(postUrl, "POST", formData),
    onMutate: () => {
      setIsLoading(true);
  ...
  const onSubmit = (data) => {
     ...
     */
    const hourFaileds = hours.filter(
      (hour) =>
        hour.checked === true &&
        (hour.ouverture === null || hour.fermeture === null)
    );
    ...
    const transformHours = hoursSubmit.map((hour) => ({
      ...hour,
      ouverture: hour.ouverture.toLocaleString(),
      fermeture: hour.fermeture.toLocaleString(),
    }));
    ...
    const postUrl = params.id
      ? `/etablissements/update/${params.id}`
      : "/etablissements/store";
    const postAlertSuccess = params.id
      ? "Établissement modifié"
      : "Enregistrement effectué";
    submitEtablissement.mutate(
      { postUrl, formData },
      {
        onSuccess: () => {
          toast.remove();
  ...
  const handleInputChange = async (e) => {
    ...
    setLoadAdress(true);
    if (value.length > 2) {
      // Lancer la recherche après quelques caractères
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?q=${value}&format=json&addressdetails=1&limit=10`
      );
  ...
  return (
    <section id="etablissements" className="content-current">
      <div className="col-md-3 mb-4">
        <a onClick={navigateBack} className="btn_3 d-inline">
          <i className="icon-right"></i> Retour
        </a>
      </div>
      {isFetching ? (
        <Loader />
            ...
            <div className="row">
                ...
                <div className="form-group">
                  <Input
                    ...
                    inputRegister={{
                      ...register("libelle", {
                        required:
                          "Veuillez saisir le libéllé de l'établissement",
                        minLength: {
                          value: "2",
                          message:
                            "Veuillez saisir un libéllé d'au moins 2 caractères",
                        },
                        maxLength: {
                          value: "100",
                          message:
                            "Le libéllé doit avoir 100 caractères maximum",
                        },
                      }),
                    }}
                  />
                ...
                <div className="form-group">
                  ...
                  <Select
                    ...
                    value={categories.find(
                      (prevCategory) => prevCategory.value === category
                    )}
                    ...
                    inputRegister={{
                      ...register("ville", {
                        ...
                        minLength: {
                          value: "2",
                          message:
                            "Veuillez saisir une ville d'au moins 2 caractères",
                        },
                        maxLength: {
                          value: "100",
                ...
                <div className="form-group">
                  <Input
                    ...
                    inputRegister={{
                      ...register("adresse", {
                        required:
                          "Veuillez saisir l'adresse de localisation de l'établissement",
                      }),
                    }}
                  />
                  {errors.adresse && (
              ...
              <div className="col-md-12">
                <div className="form-group">
                  ...
                  <Select
                    value={commodites.filter((commodite) =>
                      oldCommodites.includes(commodite.value)
                    )}
                    name="commodites"
                    options={commodites}
                    onChange={(prevCommodite) => {
                      const com = Array.from(prevCommodite).map(
                        (com) => com.value
                      );
                  ...
                  <span className="text-secondary">
                    <i className="icon-info-circled"></i>Services et
                    installations que propose votre établissement pour améliorer
                    le confort et l'expérience de vos clients
                  </span>
                </div>
              </div>
              <div className="col-md-6">
                      ...
                      <tr key={index}>
                        <td>
                          <label className="switch-light switch-ios pull-right m-0">
                            <input
                              ...
                              name="option_1"
                              id="option_1"
                              defaultChecked={hour.checked}
                              onChange={(e) =>
                                handleChangeInputCheck(hour.value, e)
                              }
                            />
                            <span>
                              <span>Non</span>
                        ...
                        <td>
                          <DatePicker
                            ...
                            className="ouverture"
                            onChange={(value) =>
                              handleChangeHour(index, "ouverture", value)
                            }
                        ...
                        <td>
                          <DatePicker
                            ...
                            className="fermeture"
                            onChange={(value) =>
                              handleChangeHour(index, "fermeture", value)
                            }
...
function ClockIcon() {
  return <span className="icon-clock-4"></span>;
}
